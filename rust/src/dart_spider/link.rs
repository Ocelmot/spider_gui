...
use spider_client::{
    beacon_lookout_many, message::{
        AbsoluteDatasetPath, DatasetData, InviteType, Message, RouterMessage, UiMessage, UiPageList,
    }, set_veilid_path_root, ClientResponse, Link, Relation, SpiderClientBuilder, SpiderId2048
};
use tokio::{
    runtime::{Builder, Runtime},
    select,
    sync::{
        broadcast::error, mpsc::{channel, Receiver, Sender}, Notify
    },
    task::JoinHandle,
};
...
use crate::dart_spider::{
    link_state::LinkState,
    ui::{DartUiInput, DartUiPage},
};
...
impl LinkProcessor {
    pub fn create(stream_sink: StreamSink<ToUi>, config_path: String) -> LinkProcessorParts {
        ...
        let processor_handle = runtime.spawn(async move {
            set_veilid_path_root(Some(config_path.into())).await;
            processor.run(notify_inner).await
        });
        ...
        loop {
            ...
            select! {
                cmd = self.receiver.recv() => {
                    ...
                    match cmd {
                        ...
                        ToProcessor::GenerateInvite => {
                            let msg = Message::Router(RouterMessage::GenerateInvite(InviteType::Veilid));
                ...
                msg = self.link_state.recv(), if self.link_state.is_connected() => {
                    info!("From base: {:?}", msg);
                    info!("link_state: {:?}", self.link_state);
                    match msg {
                        Some(msg) => {
                            match msg {
                                ...
                                ClientResponse::Message(Message::Router(RouterMessage::Invite(invite))) => {
                                ...
                                ClientResponse::Status(msg) => {
                                    self.sender.add(ToUi::Status { msg });
                                }
                                ClientResponse::Connected => {
                                    ...
                                    // subscribe to chord addrs
                                    let msg = Message::Router(RouterMessage::SubscribeChord(30));
                                    self.link_state.send(msg).await;
                                },
